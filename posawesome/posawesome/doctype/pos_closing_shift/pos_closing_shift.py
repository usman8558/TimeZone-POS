    ...
    for d in invoices:
        ...
        for p in d.payments:
            ...
            if existing_pay:
                ...
                # === NEW FORCE CALCULATE LOGIC ===
                is_cash = (existing_pay[0].mode_of_payment == cash_mode_of_payment) or ("cash" in existing_pay[0].mode_of_payment.lower())
                
                if is_cash:
                    # Agar database me change 0 hai, tou automatically (Paid - Bill) karke nikal lo
                    actual_change = flt(d.paid_amount) - flt(d.grand_total)
                    if actual_change > 0:
                        amount = flt(p.amount) - actual_change
                    else:
                        amount = flt(p.amount) - flt(d.change_amount)
                else:
                    amount = flt(p.amount)
                # === LOGIC END ===
...
def submit_printed_invoices(pos_opening_shift):
    ...
    for invoice in invoices_list:
        try:
            ...
            frappe.log_error(title=f"Failed to auto-submit Draft Invoice {invoice.name}", message=frappe.get_traceback())
