...
export default {
  ...
  computed: {
    ...
    diff_payment() {
      ...
      // Exact diff return karein (Negative value ka matlab "Change" hoga)
    ...
    credit_change() {
      // Isay hamesha 0 return karwayen taake paisa Credit me na jaye
      return 0;
    },
    ...
    diff_payment_display() {
      return this.formatCurrency(Math.abs(this.diff_payment), this.displayCurrency);
    },
  ...
  methods: {
    ...
    showDiffPayment() {
      ...
      let title = this.diff_payment > 0 
        ? `To Be Paid: ${this.formatCurrency(this.diff_payment)}` 
        : `Change: ${this.formatCurrency(Math.abs(this.diff_payment))}`;
