<template>
  ...
  <nav>
    ...
    >
      ...
      <v-toolbar-title @click="goDesk" class="text-h6 font-weight-bold navbar-title ml-4"
        style="cursor: pointer; text-decoration: none; color: #1FA4B2 !important;">
        {{ company }}
      </v-toolbar-title>
      ...
      <v-btn icon color="primary" class="mx-1" @click="hardRefresh" :title="__('Refresh Items & Clear Cache')">
...
export default {
  ...
  methods: {
    ...
    async hardRefresh() {
      // 1. Pehle check karein ke koi offline bill pending toh nahi (Data loss se bachne ke liye)
      // Note: 'pendingInvoices' variable aapke Navbar me offline badge ke liye use hota hai
      if (this.pendingInvoices > 0) {
          this.showMessage({ 
              color: 'error', 
              title: this.__('Please sync offline invoices before clearing storage!') 
          });
          return;
      }

      this.showMessage({ color: 'info', title: this.__('Deep cleaning cache and items data...') });
      
      try {
          // 2. Empty Cache (Static Assets like JS, CSS, Images)
          if ('caches' in window) {
              const cacheNames = await caches.keys();
              await Promise.all(cacheNames.map(name => caches.delete(name)));
              console.log('Static cache emptied.');
          }

          // 3. Clear Application Storage (IndexedDB - Jahan Items/Qty stuck hoti hain)
          if (window.indexedDB && window.indexedDB.databases) {
              ...
              // Fallback for older browsers or specific DB names
              const knownDBs = ['localforage', 'posawesome', 'frappe'];
              knownDBs.forEach(db => window.indexedDB.deleteDatabase(db));
          }
      ...
      // 5. Final Hard Reload (Server se fresh copy mangwane ke liye)
      setTimeout(() => {
